 ...
 * @return {Promise<void>}
 */
async function addReferences(dom: JSDOM) {
  ...
  const referencesMap: { [k: string]: HTMLAnchorElement[] } = Object.create(null);
  ...
  for (const p of document.querySelectorAll('div.post-content')) {
    ...
    for (const [referenceId, references] of Object.entries<HTMLAnchorElement[]>(referencesMap)) {
